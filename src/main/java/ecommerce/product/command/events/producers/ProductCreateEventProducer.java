...
@Slf4j
@Component
public class ProductCreateEventProducer extends AbstractProducer<String, Object, Product> {
    private final ObjectMapper objectMapper;

    @Value("${spring.kafka.topics.product}")
    private String topic;

    protected ProductCreateEventProducer(KafkaTemplate<String, Object> kafkaTemplate, ObjectMapper objectMapper) {
        super(kafkaTemplate);
    ...
    public String getKey(Product product) {
        return "PRODUCT-CRE-" + product.getId();
    }

    @Override
    public Object getValue(Product product) throws Exception {
        return objectMapper.writeValueAsString(new ProductEvent(EventType.CREATE_PRODUCT, product));
    }
